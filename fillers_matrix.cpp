...
{
public:
  MatrixSetter (std::vector<double> &A, int M1, int M2) : A (A), M1 (M1), M2 (M2), S (M1 * M2) {}
...
{
public:
  VectorSetter (std::vector<double> &B, int M1, int M2) : B (B), M1 (M1), M2 (M2) {}
...
void fill_first (int k, std::vector<double> &A, std::vector<double> &B, trio &essential)
{
  ...
  MatrixSetter A_at (A, M1, M2);
  VectorSetter B_at (B, M1, M2);
